...
- 캘린더 가져오기: 홈 '📅 캘린더' → **.ics 파일** 선택(parseICS로 VEVENT: SUMMARY/DTSTART/LOCATION/DESCRIPTION/GEO 파싱, UTC→로컬 변환) → 이벤트 선택 모달(openCalendarPicker) → createTripFromEvents가 날짜범위로 startDate·dayCount 잡고 각 이벤트를 day/time/장소(주소·lat/lng)/메모 항목으로 매핑해 새 여행 생성(createTripFromBundle 재사용). 구글/애플/아웃룩 표준 .ics 호환.
- 일정 UI 소소: 오늘 Day 섹션에 현재시각 빨간 라인(.tl-now, '현재 HH:MM' pill). 당일 지난 시각 항목은 .past(opacity .5, hover 원복).
