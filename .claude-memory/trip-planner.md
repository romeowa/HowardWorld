...
- 캘린더 내보내기(.ics): 여행 ⋯ 메뉴 '📅 캘린더에 추가(.ics)' → tripToICS()가 items를 VEVENT(시작일+day+time→DTSTART/DTEND 기본1h, 시간없으면 VALUE=DATE 종일; SUMMARY=이름, LOCATION=주소, DESCRIPTION=메모, GEO=lat;lng)로 만들어 다운로드. 맥/아이폰/구글 등에서 열면 캘린더에 추가됨. (반대로 ics '가져오기'는 웹앱이 기기 캘린더를 직접 못 읽어서, 구글 Calendar API OAuth가 필요 — 미구현/제거함.)
