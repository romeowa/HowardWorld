...
- 정산 기능: 여행 상단 '💰 정산' 버튼 → 정산 창(.settle-bg 오버레이, 사용자가 준 엑셀 정산표를 본뜬 **그리드 UI**). 참여자(trip.members) 칩 추가/삭제. 정산 내역은 **표(.exp-grid)로 인라인 편집** — 행=지출, 열=날짜·구분·내역·금액·[참여자 체크박스 N열]·결제·1인 비용(자동, 파란 셀). '＋ 행 추가'가 expenses 문서를 즉시 생성 후 셀에서 바로 입력, 셀 change 시 실시간 저장(commitExp). 표 편집 중(isGridEditing)엔 포커스 유지 위해 full re-render 대신 계산값(refreshGridComputed)+요약(renderSummaries)만 갱신. 정산표는 엑셀처럼 **이름을 열로 둔 매트릭스**(.tally: 개인별 비용/결제 합계·정산 받을(초록)/보낼(빨강) 금액) + 최소 송금 제안(그리디) + 카테고리별 합계. 데이터: trips/{id}/expenses(date·category·desc·amount·payer·sharedBy[]) 실시간 onSnapshot. 구분(trip.expenseCategories) 기본 6종(숙박/식사/교통/간식/관광/기타=DEFAULT_CATS)+행 드롭다운 '+ 새 구분'. 규칙: expenses read/write 공개(items와 동일). 창 열렸을 때만 expenses 구독(unsubExpenses)·settleRerender.
