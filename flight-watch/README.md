...
출발일/귀국일과 조건(최대 가격, 경유 횟수, 출발 시간대)을 등록해두면 Mac에서 1시간마다 구글플라이트를 크롤링하고, 조건에 맞는 항공권이 나오면 Mac/iPhone에 FCM 웹 푸시를 보내는 프로젝트.
...
2. 데몬은 시작 시 + 1시간마다 + 웹 UI "지금 체크" 클릭 시(`control/checkNow` 감지) 전체 감시 검사
...
- 크롤링 간격을 너무 줄이면(분 단위) 구글이 차단할 수 있음 — 1시간 권장
